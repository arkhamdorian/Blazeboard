...
void insert(trie* head[], string parent, int type, int size_of = 1)
{
	...
	if (parent.size() >= size_of)
	{
		...
		else
		{
			...
			::curr = head[t_p];
			insert(head[t_p]->words, parent, type, size_of + 
1);
		}
	}
	else
	{
		...
		else
		{
			...
			//err << "currentword:" << ::curr->content << 
endl;

			for (int i = 0; i < ::prev->links.size() && f == 
-1; i++)
			{
				//err << ::curr << ' ' << 
::prev->links[i].first << ' ' << ::prev->links[i].first->content << 
endl;
			...
			{
				::prev->links.push_back(pair<trie*, 
int>(::curr, 1));
			}
			sort(::prev->links.begin(), ::prev->links.end(), 
compare);
			::prev = ::curr;
			err.close();
		}
...
void Sort(trie* words[])
{
	for (int i = 0; i<26; ++i)
	{
		if (words[i] != NULL)
		{
			if (words[i]->complete)
			{
				dict.insert(pair<int, 
trie*>(words[i]->prob, words[i]));
			}
			Sort(words[i]->words);
		}
	}
...
void nextword(char *str1, char *str2, char *str3, int choice = 0)
{
	...
	for (int i = 0; i<t; i++)
	{
		...
		{
			strcpy_s(str1, 30, 
::curr->links[i].first->content.c_str());
		}
		if (i == 1)
		{
			strcpy_s(str2, 30, 
...
extern "C"
{
	...
	__declspec(dllexport) void Space(const char*prev, char*str1, 
char*str2, char*str3, int choice)
	{
		insert(head, prev, 1);
		nextword(str1, str2, str3, choice);
	}
	__declspec(dllexport) void Search(const char *text, char*str1, 
char*str2, char*str3)
	{
